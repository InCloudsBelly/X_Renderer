...
namespace X {
    ...
    namespace Renderer {
        ...
        void ShadowRenderer::Render(Viewport* viewport, RenderTarget* target, Camera* camera,
            ...
            // PointLight Offset and transfer Layout
            uint32_t pointLightOffset = 0;

            commandList->ImageMemoryBarrier(
                Lighting::PointLight::s_pointLightsArrayShadow->cubeArray.image,
            ...
            for (auto& light : lights) {
                ...
                switch (light->type)
                {
                    case X_DIRECTIONAL_LIGHT: componentCount = 1; break;
                    case X_POINT_LIGHT: componentCount = 6; break;
                }

                bool isDirectionalLight = false;
                vec3 lightLocation;

                if (light->type == X_DIRECTIONAL_LIGHT) {
                ...
                else if (light->type == X_POINT_LIGHT) {
                ...
                for (uint32_t i = 0; i < uint32_t(componentCount); i++) {
                    ...
                    Lighting::ShadowComponent* component;
                    int layerIndex;
                    if (light->type == X_DIRECTIONAL_LIGHT)
                    {
                        component = &shadow->components[0];
                        layerIndex = 0;
                    }   
                    else if (light->type == X_POINT_LIGHT)
                    {
                        component = &Lighting::PointLight::s_pointLightShadowComponents[pointLightOffset + i];
                        layerIndex = pointLightOffset + i;
                    }

                    if (frameBuffer->depthAttachment.layer != layerIndex) {
                        frameBuffer->depthAttachment.layer = layerIndex;
                        frameBuffer->Refresh();
                    }
        ...
        Ref<Graphics::FrameBuffer> ShadowRenderer::GetOrCreateFrameBuffer(Lighting::Light* light) {
            ...
            if (light->type == X_DIRECTIONAL_LIGHT)
                ...
                shadow = Lighting::PointLight::s_pointLightsArrayShadow;
            ...
            if (lightMap.contains(light)) {
                ...
                //};
                Graphics::RenderPassDepthAttachment attachment = {
                   .imageFormat = light->type == X_DIRECTIONAL_LIGHT ? shadow->maps.format : shadow->cubeArray.format,
                   .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
                   .initialLayout = VK_IMAGE_LAYOUT_UNDEFINED,
                   .outputLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
                };
                ...
                Graphics::FrameBufferDesc frameBufferDesc = {
                    .renderPass = renderPass,
                    .depthAttachment = { light->type == X_DIRECTIONAL_LIGHT ? shadow->maps.image : shadow->cubeArray.image, 0, true},
                    .extent = { uint32_t(shadow->resolution), uint32_t(shadow->resolution) }
                };
        ...
        PipelineConfig ShadowRenderer::GetPipelineConfigForSubData(Mesh::MeshSubData *subData,
            ...
            return PipelineConfig("ShadowMapping_Pipeline", shaderConfig, pipelineDesc, macros);
