...
#define FEATURE_EMISSION_MAP (1 << 7)
#define FEATURE_TRANSMISSION (1 << 8)
#define FEATURE_VERTEX_COLORS (1 << 9)
...
namespace X {
    ...
    namespace Renderer {
        ...
        void MainRenderer::Init(Graphics::GraphicsDevice *device) {
            ...
            pointLightsUniformBuffer = CreateRef<Buffer::UniformBuffer>(16ull + sizeof(PointLight) * MAX_POINTLIGHT_SHADOW_COUNT);
            ...
            lightcullingRenderer.Init(device);
        ...
        void MainRenderer::RenderScene(Viewport* viewport, RenderTarget* target, Camera* camera, 
            ...
            {
                // -------------- Point Light UniformBuffer ------------------------------
                auto Lights = scene->GetLights();

                commandList->BindImage(Lighting::PointLight::s_pointLightsArrayShadow->cubeArray.image, Lighting::PointLight::s_pointLightsArrayShadow->cubeArray.sampler, 2, 28);

                PointLightUniforms pointUniforms;
                pointUniforms.PointLightCount = 0;
                for (auto light : Lights)
                {
                    if (light->type == X_POINT_LIGHT)
                    {
                        int index = pointUniforms.PointLightCount;
                        assert(index < MAX_POINTLIGHT_SHADOW_COUNT, "Now Only Support 16 PointLights max");

                        auto pointLight = dynamic_cast<Lighting::PointLight*>(light);
                        pointUniforms.PointLights[index].color = vec4(pointLight->color, 1.0f);
                        pointUniforms.PointLights[index].loacation = vec4(pointLight->location, 1.0f);
                        pointUniforms.PointLights[index].radius = pointLight->radius;
                        pointUniforms.PointLights[index].MinRadius = 0.1f;
                        pointUniforms.PointLights[index].intensity = pointLight->intensity;
                        pointUniforms.PointLightCount++;
                    }
                }
                pointLightsUniformBuffer->SetData(&pointUniforms, 0);
                pointLightsUniformBuffer->Bind(commandList, 2, 27);
                // -------------- Point Light UniformBuffer -----------------------------
...

            {
                ...
                auto shadow = scene->sky.sun->GetShadow();

                if (shadow) {
                    imageBarriers.push_back({  shadow->maps.image, layout, access });
                }


                //PointLights
                imageBarriers.push_back({ Lighting::PointLight::s_pointLightsArrayShadow->cubeArray.image, layout, access });


                commandList->PipelineBarrier(imageBarriers, bufferBarriers, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT);
            }

            lightcullingRenderer.Render(viewport, target, camera, scene, commandList);
            ...
            commandList->BindImage(targetData->emissionTexture->image, targetData->emissionTexture->sampler, 1, 9);
            ...
            if (!target->HasHistory()) {
                ...
                std::vector<Graphics::ImageBarrier> imageBarriers = {
                    ...
                    {rtData->emissionTexture->image, layout, access},
        ...
        void MainRenderer::RenderProbe(Lighting::EnvironmentProbe* probe, RenderTarget* target, Scene::Scene* scene) {
            ...
            auto materialBuffer = Buffer::Buffer(X_SHADER_STORAGE_BUFFER, sizeof(PackedMaterial), 0,
        ...
        void MainRenderer::FilterProbe(Lighting::EnvironmentProbe* probe, Graphics::CommandList* commandList) {
            ...
            auto pipelineConfig = PipelineConfig("FilterDiffuseProbe_Pipeline","brdf/filterProbe.csh", { "FILTER_DIFFUSE" });
            ...
            pipelineConfig = PipelineConfig("FilterSpecularProbe_Pipeline", "brdf/filterProbe.csh", { "FILTER_SPECULAR" });
        ...
        void MainRenderer::PrepareMaterials(Scene::Scene* scene, std::vector<PackedMaterial>& materials,
            ...
            for (auto material : sceneMaterials) {
                ...
                packed.features |= material->HasEmissionMap() ? FEATURE_EMISSION_MAP : 0;
        ...
        void MainRenderer::FillRenderList(Scene::Scene *scene, X::Camera *camera) {
            ...
            for (auto light : lights) {
                ...
                for (int32_t i = 0; i < componentCount; i++) {
                    ...
                    auto frustum = Volume::Frustum(component->projectionMatrix * component->viewMatrix);
        ...
        void MainRenderer::PreintegrateBRDF() {
            ...
            auto pipelineConfig = PipelineConfig("Preintegrate_BRDG_Pipeline", "brdf/preintegrateDFG.csh");
