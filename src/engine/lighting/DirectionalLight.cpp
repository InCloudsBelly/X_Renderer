...
namespace X {
    ...
    namespace Lighting {
        ...
        DirectionalLight::DirectionalLight(int32_t mobility) : Light(X_DIRECTIONAL_LIGHT, mobility) {
        ...
        void DirectionalLight::AddShadow(float distance, float bias, int32_t resolution, vec3 centerPoint, mat4 orthoProjection) {
            ...
            glm::vec3 up = std::abs(glm::dot(direction, glm::vec3(0.f, 1.0f, 0.0f))) > 0.9 ? glm::normalize(glm::vec3(0.01f, 1.0f, 0.01f)) : glm::vec3(0.f, 1.0f, 0.0f);
            shadow->components[0].viewMatrix = glm::lookAt(centerPoint, centerPoint + direction, up);
