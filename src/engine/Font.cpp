...
namespace X {
    ...
    Font::Font(std::string filename, float pixelSize, int32_t padding, uint8_t edgeValue) : edgeValue(edgeValue) {
        ...
        glyphs.resize(X_FONT_GLYPH_COUNT);
        glyphInfo.resize(X_FONT_GLYPH_COUNT);

        glyphBuffer = Buffer::Buffer(Buffer::BufferUsageBits::StorageBufferBit, sizeof(GlyphInfo), X_GPU_GLYPH_COUNT);
        ...
        while (glyphCount < font.numGlyphs && codepointCount < X_FONT_GLYPH_COUNT) {
        ...
        codepointCount = codepointCount > X_FONT_GLYPH_COUNT ? X_FONT_GLYPH_COUNT : codepointCount;

        uint8_t *data[X_FONT_GLYPH_COUNT];

        int32_t width = 0, height = 0, depth = 0;
        ivec2 offset = ivec2(0, 0);

        // Initialize the kern for every glyph possible
        for (int32_t i = 0; i < X_FONT_GLYPH_COUNT; i++)
            glyphs[i].kern.resize(X_FONT_GLYPH_COUNT);
        ...
        for (int32_t i = font.fontstart; i < codepointCount; i++) {
            ...
            for (int32_t j = 0; j < X_FONT_GLYPH_COUNT; j++) {
            ...
            if (!data[i]) {
                ...
                glyph->texArrayIndex = X_GPU_GLYPH_COUNT;
                continue;
            }
        ...
        depth = X_GPU_GLYPH_COUNT < depth ? X_GPU_GLYPH_COUNT : depth;
        ...
        glyphBuffer.SetData(&glyphInfo[0], 0, X_GPU_GLYPH_COUNT);
    ...
    Glyph *Font::GetGlyph(char character) {
        ...
        if (characterIndex > X_FONT_GLYPH_COUNT)
            return nullptr;
