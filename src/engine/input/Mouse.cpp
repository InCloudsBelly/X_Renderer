...
namespace X {
    ...
    namespace Input {
        ...
        MouseHandler::~MouseHandler() {
            ...
            X::Events::EventManager::MouseMotionEventDelegate.Unsubscribe(
                mouseMotionEventHandle
            );
            X::Events::EventManager::MouseButtonEventDelegate.Unsubscribe(
        ...
        void MouseHandler::MouseButtonEventHandler(Events::MouseButtonEvent event) {
            ...
            if (event.button == activationButton && event.state == X_BUTTON_PRESSED)
            {
                activationButtonDown = true;
                HideMouse();
            }
            if (event.button == activationButton && event.state == X_BUTTON_RELEASED)
            {
