...
namespace std {
	...
	template <>
	struct hash<X::Common::UUID>
	{
		std::size_t operator()(const X::Common::UUID& uuid) const
		{
	...
	template <>
	struct hash<X::Common::UUID32>
	{
		std::size_t operator()(const X::Common::UUID32& uuid) const
		{
