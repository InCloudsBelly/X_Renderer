...
namespace X {
    ...
    Window::Window(const std::string& title, int32_t x, int32_t y, int32_t width, int32_t height,
            int32_t flags, bool createSurface) : title(title), x(x == X_WINDOWPOSITION_UNDEFINED ? 0 : x),
            y(y == X_WINDOWPOSITION_UNDEFINED ? 0 : y), width(width), height(height) {
    ...
    void Window::SetFullscreen(bool fullscreen) {
        ...
        SDL_SetWindowFullscreen(sdlWindow, (fullscreen ? X_WINDOW_FULLSCREEN : 0));
    ...
    void Window::WindowEventHandler(Events::WindowEvent event) {
        ...
        if (event.type == X_WINDOWEVENT_FOCUS_GAINED) {
            hasFocus = true;
        }
        ...
        if (event.type == X_WINDOWEVENT_RESIZED) {
            width = event.data.x;
            height = event.data.y;
        }
    ...
    void Window::DropEventHandler(X::Events::DropEvent event) {
