...
namespace X {
    ...
    namespace Loader {
        ...
        void ModelLoader::LoadMaterial(aiMaterial* assimpMaterial, MaterialImages& images, Material& material) {
            ...
            if (images.emissionImage && images.emissionImage->HasData()) {
                material.emissionMap = std::make_shared<Texture::Texture2D>(images.emissionImage);
                material.emissionMapPath = images.emissionImage->fileName;
            }
        ...
        void ModelLoader::LoadMaterialImages(aiMaterial* material, MaterialImages& images, const std::string& directory,
            ...
            {
                aiString aiPath;
                if (material->GetTexture(AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_METALLICROUGHNESS_TEXTURE, &aiPath) == AI_SUCCESS)
                {
                    auto path = Common::Path::Normalize(directory + std::string(aiPath.C_Str()));
                    images.roughnessImage = ImageLoader::LoadImage<uint8_t>(path, false, 0, maxTextureResolution);
                    if (images.roughnessImage->channels > 1) {
            ...
            if (material->GetTextureCount(aiTextureType_EMISSIVE) > 0 ) {
                ...
                auto path = Common::Path::Normalize(directory + std::string(aiPath.C_Str()));
                images.emissionImage = ImageLoader::LoadImage<uint8_t>(path, false, 3, maxTextureResolution);
            }
        }
