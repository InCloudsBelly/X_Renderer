...
namespace X {
    ...
    namespace Scene {
            ...
            for (auto& mesh : meshes) {
                ...
                for (auto& material : mesh->data.materials) {
                    ...
                    if (material.HasEmissionMap()) {
                        auto& slices = emissionTextureAtlas.slices[material.emissionMap.get()];

                        gpuMaterial.emissionTexture = CreateGPUTextureStruct(slices);
                    }
        ...
        void RTData::UpdateTextures() {
            ...
            std::vector<Ref<Texture::Texture2D>> emissionTextures;
            ...
            for (auto& mesh : meshes) {
                ...
                for (auto& material : mesh->data.materials) {
                    ...
                    if (material.HasEmissionMap())
                        emissionTextures.push_back(material.emissionMap);
                }
            }
            ...
            emissionTextureAtlas = Texture::TextureAtlas(emissionTextures, 1, textureDownscale);
