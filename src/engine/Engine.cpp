...
extern X::EngineInstance* GetEngineInstance();
...
namespace X {
    ...
    void Engine::Init(EngineConfig config) {
        ...
        DefaultWindow = new Window("Default window", X_WINDOWPOSITION_UNDEFINED,
            X_WINDOWPOSITION_UNDEFINED, 100, 100,
            SDL_WINDOW_VULKAN | X_WINDOW_HIDDEN, false);

        // Then create graphics instance
#ifdef X_BUILDTYPE_RELEASE
        Graphics::Instance::DefaultInstance = new Graphics::Instance("X3EngineInstance", false);
#else
        Graphics::Instance::DefaultInstance = new Graphics::Instance("X3EngineInstance", true);
#endif
