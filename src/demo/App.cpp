...
const X::EngineConfig X::EngineInstance::engineConfig = {
    .assetDirectory = "../../data",
...
void App::LoadContent() {
    ...
    renderTarget = X::RenderTarget(1920, 1080);
    pathTraceTarget = X::Renderer::PathTracerRenderTarget(1920, 1080);

    auto icon = X::Texture::Texture2D("X_logo_50x50.png");
    window.SetIcon(&icon);

    loadingTexture = X::CreateRef<X::Texture::Texture2D>("X_logo_100x100.png");

    font = X::Font("font/roboto.ttf", 22, 5);

    camera = X::Camera(47.0f, 2.0f, 0.1f, 200.0f,
        glm::vec3(30.0f, 25.0f, 0.0f), glm::vec2(-3.14f / 2.0f, 0.0f));

    scene = X::CreateRef<X::Scene::Scene>(glm::vec3(-2048.0f), glm::vec3(2048.0f));

    mouseHandler = X::Input::MouseHandler(&camera, 2.3f, 20.0f);
    mouseHandler.SetActivationButton(X_MOUSEBUTTON_RIGHT);
    keyboardHandler = X::Input::KeyboardHandler(&camera, 14.0f, 12.0f);

    X::Events::EventManager::KeyboardEventDelegate.Subscribe(
        [this](X::Events::KeyboardEvent event) {
            if (event.keyCode == X_KEY_ESCAPE) {
                Exit();
            }
            if (event.keyCode == X_KEY_F11 && event.state == X_BUTTON_RELEASED) {
                renderUI = !renderUI;
            }
            if (event.keyCode == X_KEY_LSHIFT && event.state == X_BUTTON_PRESSED) {
                keyboardHandler.speed = cameraSpeed * 4.0f;
            }
            if (event.keyCode == X_KEY_LSHIFT && event.state == X_BUTTON_RELEASED) {
                keyboardHandler.speed = cameraSpeed;
            }
        });

    directionalLight = std::make_shared<X::Lighting::DirectionalLight>(X_MOVABLE_LIGHT);
    directionalLight->direction = glm::vec3(0.0f, -1.0f, 1.0f);
    directionalLight->color = glm::vec3(255, 236, 209) / 255.0f;
    glm::mat4 orthoProjection = glm::ortho(-50.0f, 50.0f, -50.0f, 50.0f, -240.0f, 100.0f);
    ...
    pointLights.push_back(std::make_shared<X::Lighting::PointLight>(X_MOVABLE_LIGHT, 10.0f));
    pointLights.back()->AddShadow(3.0f, 512);
    scene->Add(pointLights.back().get());
    ...
    scene->ao = std::make_shared<X::Lighting::AO>(16);
    scene->ao->rt = true;
    scene->reflection = std::make_shared<X::Lighting::Reflection>(1);
    scene->reflection->useShadowMap = true;

    scene->fog = std::make_shared<X::Lighting::Fog>();
    ...
    scene->sky.clouds = std::make_shared<X::Lighting::VolumetricClouds>();
    scene->sky.clouds->minHeight = 100.0f;
    scene->sky.clouds->maxHeight = 600.0f;
    scene->sky.clouds->castShadow = false;

    scene->sky.atmosphere = std::make_shared<X::Lighting::Atmosphere>();

    scene->postProcessing.taa = X::PostProcessing::TAA(0.99f);
    scene->postProcessing.sharpen.enable = true;
    scene->postProcessing.sharpen.factor = 0.15f;

    scene->sss = std::make_shared<X::Lighting::SSS>();
...
void App::Update(float deltaTime) {
    ...
    if (rotateCamera) {
        camera.rotation.y += rotateCameraSpeed * cos(X::Clock::Get());
        mouseHandler.Reset(&camera);
    }

    if(moveCamera) {
        camera.location += camera.right * moveCameraSpeed * cos(X::Clock::Get());
        mouseHandler.Reset(&camera);
    }
    ...
    if (sceneSelection == SPONZA) {
        ...
        static float lastTime = X::Clock::Get();

        float curTime = X::Clock::Get();
        ...
        if (!initialized)
        {
            ...
            actors[0].SetMatrix(transform);

            //actors[1].SetMatrix(matrix);
            actors[1].SetTranslation(glm::vec3(-17.0f, 0.0f, 0.0f));
            actors[1].SetScale(glm::vec3(0.4f));


            //actors[2].SetMatrix(matrix);
            actors[2].SetTranslation(glm::vec3(0.0f, 17.0f, .0f));

            transform = glm::translate(glm::mat4(1.0f), glm::vec3(-25, 3.f, 0));
            transform = glm::scale(transform, glm::vec3(2.5f));
            actors[3].SetMatrix(transform);

            initialized = true;
        }
        actors[1].SetEulerRotation(actors[1].GetRotation() + glm::vec3(0.f, delta / 4.0f, 0.0f));
        actors[2].SetTranslation(actors[2].GetTranslation() + glm::vec3(0.f, 10.f*(sin(curTime/ 2.0f) - sin(lastTime / 2.0f)), 0.0f));
...
void App::Render(float deltaTime) {
    ...
    if (windowFlags & X_WINDOW_HIDDEN || windowFlags & X_WINDOW_MINIMIZED || !(windowFlags & X_WINDOW_SHOWN)) {
        return;
    }
    ...
    if (animateLight) directionalLight->direction = glm::vec3(0.0f, -1.0f, sin(X::Clock::Get() / 10.0f));
    ...
    if (pathTrace) {
        ...
        if (debug) {
            auto commandList = graphicsDevice->GetCommandList(X::Graphics::GraphicsQueue);
            ...
            if (debugAo) {
                ...
                mainRenderer->textureRenderer.RenderTexture2D(commandList, &viewport,
                    renderTarget.GetData(X::FULL_RES)->velocityTexture.get(),
    ...
    float averageFramerate = X::Clock::GetAverage();
        ...
        if (ImGui::Begin("Settings", (bool*)0, ImGuiWindowFlags_HorizontalScrollbar)) {
            ...
            if (ImGui::CollapsingHeader("General")) {
                ...
                if (vsync != vsyncMode || hdr != hdrMode) {
                    X::Graphics::ColorSpace colorSpace = X::Graphics::SRGB_NONLINEAR;
                    if (hdr) {
                        colorSpace = X::Graphics::HDR10_HLG;
                    }
            ...
            if (ImGui::CollapsingHeader("Profiler")) {
                ...
                X::Graphics::Profiler::OrderBy order;
                switch (item) {
                case 1: order = X::Graphics::Profiler::OrderBy::MAX_TIME; break;
                case 2: order = X::Graphics::Profiler::OrderBy::MIN_TIME; break;
                default: order = X::Graphics::Profiler::OrderBy::CHRONO; break;
                }

                std::function<void(X::Graphics::Profiler::Query&)> displayQuery;
                displayQuery = [&displayQuery](X::Graphics::Profiler::Query& query) {
                    ImGui::TableNextRow();
                ...
                if (ImGui::BeginTable("PerfTable", 2, flags))
                {
                    ...
                    auto threadData = X::Graphics::Profiler::GetQueriesAverage(64, order);
            ...
            if (ImGui::CollapsingHeader("Actors")) {
                ...
                ImGui::Indent(); // Ëõ½ø×ÓÕÛµþ¿ò
                for (auto& actor : actors)
                {
                    if (ImGui::CollapsingHeader(actor.name.c_str())) {
                        glm::vec3 pos = actor.GetTranslation();
                        ImGui::SliderFloat3(("Position##" + actor.name).c_str(), (float*)&pos, -20.0f, 20.f);
                        actor.SetTranslation(pos);
                        //ImGui::SliderFloat3("Max", (float*)&volume->aabb.max, -200.0f, 200.0f);
                    }
                }

                ImGui::Unindent(); // »Ö¸´Ëõ½ø
            }
            ...
            if (ImGui::CollapsingHeader("Light")) {
                ...
                for (uint32_t i = 0; i < pointLights.size(); ++i )
                {
                    if (ImGui::CollapsingHeader(("PointLight" + std::to_string(i)).c_str())) {
                        X::Lighting::PointLight& pointLight = *pointLights[i];
                        ImGui::SliderFloat3(("Position##" + std::to_string(i)).c_str(), (float*)&pointLight.location, -20.0f, 20.f);
                        ImGui::SliderFloat3(("Color##" + std::to_string(i)).c_str(), (float*)&pointLight.color, 0.f, 1.f);
                        ImGui::SliderFloat(("Intensity##" + std::to_string(i)).c_str(), (float*)&pointLight.intensity, 0.f, 50.f);
                        ImGui::SliderFloat(("Radius##" + std::to_string(i)).c_str(), (float*)&pointLight.radius, 0.f, 20.f);
                        ImGui::Separator();
                    }
                }
            ...
            if (ImGui::CollapsingHeader("Fog")) {
                ...
                ImGui::Text("Volumetric Rendering");
                ImGui::Checkbox("Enable##Volumetric", &fog->renderVolume);

                static int choice = 1;
                ImGui::RadioButton("rayMarcing Volume (disable fog for better result)", &choice, 0);
                ImGui::RadioButton("Froxel Volume", &choice, 1);

                if (choice == 1) {
                    fog->useFroxelVolume = true;
                    fog->useRayMarchingVolume = false;
                }
                else if (choice == 0) {
                    fog->useFroxelVolume = false;
                    fog->useRayMarchingVolume = true;
                }

                ImGui::Checkbox("EnableJitter##FroxelFog", &fog->froxelEnableJitter);
                ImGui::Checkbox("EnableTemperalAccumulating##FroxelFog", &fog->froxelEnableTemperalAccumulating);

                fog->froxelColor = glm::pow(fog->froxelColor, 1.0f / glm::vec3(2.2f));
                ImGui::ColorEdit3("Color##FroxelFog", &fog->froxelColor[0]);
                fog->froxelColor = glm::pow(fog->froxelColor, glm::vec3(2.2f));

                ImGui::SliderFloat("Anisotropy##FroxelFog", &fog->froxelFogAnisotropy, -1.0f, 1.0f, "%.2f", ImGuiSliderFlags_Logarithmic);
                ImGui::SliderFloat("Density##FroxelFog", &fog->froxelFogGlobalDensity, 0.0f, 2.0f, "%.2f", ImGuiSliderFlags_Logarithmic);
                ImGui::SliderFloat("LightMul##FroxelFog", &fog->froxelFogLightMul, 0.0f, 5.0f, "%.2f", ImGuiSliderFlags_Logarithmic);


                ImGui::SliderFloat("Falloff##HeightFog", &fog->froxelHeightFogExponent, 0.0f, 5.0f, "%.2f", ImGuiSliderFlags_Logarithmic);
                ImGui::SliderFloat("Offset##HeightFog", &fog->heightFalloff, 0.0f, 5.0f, "%.2f", ImGuiSliderFlags_Logarithmic);
                ImGui::SliderFloat("FogAmount##HeightFog", &fog->froxelHeightFogAmount, 0.0f, 5.0f, "%.2f", ImGuiSliderFlags_Logarithmic);
...
void App::DisplayLoadingScreen(float deltaTime) {
    ...
    rotation += deltaTime * abs(sin(X::Clock::Get())) * 10.0f;
...
bool App::IsSceneAvailable(SceneSelection selection) {
    switch (selection) {
        case CORNELL: return X::Loader::AssetLoader::FileExists("cornell/CornellBox-Original.obj");
        case SPONZA: return X::Loader::AssetLoader::FileExists("sponza1/sponza.gltf");
        case BISTRO: return X::Loader::AssetLoader::FileExists("bistro/mesh/exterior.obj");
        case SANMIGUEL: return X::Loader::AssetLoader::FileExists("sanmiguel/san-miguel-low-poly.obj");
        case MEDIEVAL: return X::Loader::AssetLoader::FileExists("medieval/scene.fbx");
        case PICAPICA: return X::Loader::AssetLoader::FileExists("pica pica/mesh/scene.gltf");
        case SUBWAY: return X::Loader::AssetLoader::FileExists("subway/scene.gltf");
        case MATERIALS: return X::Loader::AssetLoader::FileExists("material demo/materials.obj");
        case FOREST: return X::Loader::AssetLoader::FileExists("forest/forest.gltf");
        case NEWSPONZA: return X::Loader::AssetLoader::FileExists("newsponza/main/NewSponza_Main_Blender_glTF.gltf") &&
                               X::Loader::AssetLoader::FileExists("newsponza/candles/NewSponza_100sOfCandles_glTF_OmniLights.gltf") &&
                               X::Loader::AssetLoader::FileExists("newsponza/curtains/NewSponza_Curtains_glTF.gltf") &&
                               X::Loader::AssetLoader::FileExists("newsponza/ivy/NewSponza_IvyGrowth_glTF.gltf");
...
bool App::LoadScene() {
    ...
    using namespace X::Loader;

    if (sceneSelection == CORNELL) {
        meshes.reserve(1);

        glm::mat4 transform = glm::scale(glm::mat4(1.0f), glm::vec3(10.0f));
        auto mesh = X::ResourceManager<X::Mesh::Mesh>::GetResourceWithLoaderAsync(
        ...
        glm::mat4 transform = glm::scale(glm::mat4(1.0f), glm::vec3(1.f));
        auto mesh = X::ResourceManager<X::Mesh::Mesh>::GetResourceWithLoaderAsync(
            "sponza1/sponza.gltf" , ModelLoader::LoadMesh, false, transform, 2048
        );
        meshes.push_back(mesh);
 
   
        transform = glm::scale(glm::mat4(1.0f), glm::vec3(1.f));
        mesh = X::ResourceManager<X::Mesh::Mesh>::GetResourceWithLoaderAsync(
        ...
        transform = glm::scale(glm::mat4(1.0f), glm::vec3(1.f));
        mesh = X::ResourceManager<X::Mesh::Mesh>::GetResourceWithLoaderAsync(
            "damagedHelmet/DamagedHelmet.gltf", ModelLoader::LoadMesh, false, transform, 2048
        );
        meshes.push_back(mesh);
        ...
        pointLights[0]->color = glm::vec3(1.0f, 0.0f, 0.0f);
        pointLights[0]->location = glm::vec3(0.2, 2.0f, 0.0f);
        pointLights[0]->intensity = 1.5f;
        pointLights[0]->radius = 10.f;

        pointLights[1]->color = glm::vec3(0.0f, 1.0f, 0.0f);
        pointLights[1]->location = glm::vec3(-19.f, 2.0f, 0.0f);
        pointLights[1]->intensity = 1.5f;
        pointLights[1]->radius = 10.f;
        ...
        sky = X::Texture::Cubemap("environment.hdr", 2048);
        probe = X::Lighting::EnvironmentProbe(sky);
        scene->sky.probe = X::CreateRef(probe);
        ...
        auto otherScene = X::Loader::ModelLoader::LoadScene("forest/forest.gltf");
    ...
    // scene.sky.probe = std::make_shared<X::Lighting::EnvironmentProbe>(sky);
    ...
    if (sceneSelection != FOREST) {
        ...
        for (auto &mesh: meshes) {
            ...
            std::string name = X::Common::Path::GetFileName(mesh.GetResource()->path);
            actors.push_back(X::Actor::MovableMeshActor{ name, mesh, glm::translate(glm::mat4(1.0f),
                glm::vec3(0.0f))});

            /*
            if (meshCount == 1) {
                for (int32_t i = 0; i < 20000; i++) {
                    auto x = (2.0f * X::Common::Random::SampleFastUniformFloat() - 1.0f) * 100.0f;
                    auto y = (2.0f * X::Common::Random::SampleFastUniformFloat() - 1.0f) * 100.0f;
                    auto z = (2.0f * X::Common::Random::SampleFastUniformFloat() - 1.0f) * 100.0f;

                    actors.push_back(X::Actor::MovableMeshActor{mesh, glm::translate(glm::mat4(1.0f),
                        glm::vec3(x, y, z))});
                }
            }
            */
...
void App::CheckLoadScene() {
    ...
    auto sceneAABB = X::Volume::AABB(glm::vec3(std::numeric_limits<float>::max()),
    ...
    if (sceneSelection == CORNELL) {
        scene->irradianceVolume = std::make_shared<X::Lighting::IrradianceVolume>(
        ...
        scene->irradianceVolume->strength = 3.0f;
    }
    else if (sceneSelection == BISTRO) {
        scene->irradianceVolume = std::make_shared<X::Lighting::IrradianceVolume>(
            sceneAABB.Scale(0.9f), glm::ivec3(20));
    ...
    auto device = X::Graphics::GraphicsDevice::DefaultDevice;
...
void App::CopyActors(X::Ref<X::Scene::Scene> otherScene) {

    auto otherActors = otherScene->GetMeshActors();

    for (auto actor : otherActors) {

        actors.push_back(X::Actor::MovableMeshActor{actor->name, actor->mesh, actor->globalMatrix});
...
X::EngineInstance* GetEngineInstance() {
