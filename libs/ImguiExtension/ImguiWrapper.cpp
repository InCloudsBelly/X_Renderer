...
void ImguiWrapper::Load(X::Window* window) {

     // Setup back-end capabilities flags
    ImGuiIO& io = ImGui::GetIO();
    io.BackendFlags |= ImGuiBackendFlags_HasMouseCursors;       // We can honor GetMouseCursor() values (optional)
    io.BackendFlags |= ImGuiBackendFlags_HasSetMousePos;        // We can honor io.WantSetMousePos requests (optional, rarely used)
    io.BackendPlatformName = "imgui_impl_X";
    ...
    */
    ...
    keyboardID = X::Events::EventManager::KeyboardEventDelegate.Subscribe(keyboardEventHandler);

    auto textInputEventHandler = std::bind(&ImguiWrapper::TextInputHandler, this, std::placeholders::_1);
    textInputID = X::Events::EventManager::TextInputEventDelegate.Subscribe(textInputEventHandler);
...
void ImguiWrapper::Unload() {
    ...
    auto instance = X::Graphics::Instance::DefaultInstance;
...
void ImguiWrapper::Update(X::Window* window, float deltaTime) {
    ...
    if (io.WantTextInput)
        X::Events::EventManager::EnableTextInput();
    else
        X::Events::EventManager::DisableTextInput();
...
void ImguiWrapper::Render(bool clearSwapChain) {
    ...
    auto commandList = device->GetCommandList(X::Graphics::GraphicsQueue);

    commandList->BeginCommands();

    X::Graphics::Profiler::BeginThread("ImGui thread", commandList);
    X::Graphics::Profiler::BeginQuery("ImGui");
    ...
    X::Graphics::Profiler::EndQuery();
    X::Graphics::Profiler::EndThread();
...
void ImguiWrapper::RecreateImGuiResources() {
    ...
    auto queue = device->GetAndLockQueue(X::Graphics::GraphicsQueue);
    ...
    device->memoryManager->transferManager->ImmediateSubmit(X::Graphics::GraphicsQueue,
        [&](X::Graphics::CommandList* commandList) {
...
VkDescriptorSet ImguiWrapper::GetTextureDescriptorSet(const X::Texture::Texture2D& texture,
...
void ImguiWrapper::WindowHandler(X::Events::WindowEvent event) {

    ImGuiIO& io = ImGui::GetIO();

    if (event.type == X_WINDOWEVENT_FOCUS_LOST) {
        io.MousePos = ImVec2(-FLT_MAX, -FLT_MAX);
    }

}

void ImguiWrapper::MouseButtonHandler(X::Events::MouseButtonEvent event) {

    ImGuiIO& io = ImGui::GetIO();

    int mouseButton = -1;
    if (event.button == X_MOUSEBUTTON_LEFT) { mouseButton = 0; }
    if (event.button == X_MOUSEBUTTON_RIGHT) { mouseButton = 1; }
    if (event.button == X_MOUSEBUTTON_MIDDLE) { mouseButton = 2; }
...
void ImguiWrapper::MouseMotionHandler(X::Events::MouseMotionEvent event) {
...
void ImguiWrapper::MouseWheelHandler(X::Events::MouseWheelEvent event) {
...
void ImguiWrapper::KeyboardHandler(X::Events::KeyboardEvent event) {
...
void ImguiWrapper::TextInputHandler(X::Events::TextInputEvent event) {
