...
vec3 ComputeVolumetric(vec3 fragPos, vec2 texCoords) {
    ...
    for (int i = 0; i < uniforms.sampleCount; i++) {
        ...
        float fogAmount = uniforms.fogEnabled > 0 ? (saturate(1.0 - ComputeVolumetricFog(uniforms.fog, globalData.cameraLocation.xyz, worldPosition))) * 10.0f: 1.0;
        float NdotL = dot(rayDirection, uniforms.light.direction.xyz);

        float scattering = uniforms.fogEnabled > 0 ? uniforms.fog.scatteringAnisotropy : -0.7;
        foginess += shadowValue * fogAmount * ComputeScattering(scattering, NdotL) * uniforms.light.color.rgb * (uniforms.fogEnabled > 0 ? uniforms.fog.color.rgb : vec3(1.0));
