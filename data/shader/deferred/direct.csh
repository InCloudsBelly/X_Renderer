...
#ifdef SHADOWS
layout(set = 3, binding = 1) uniform sampler2DArray cascadeMaps;
#endif
...
void main() {
    ...
    if (depth < 1.0) {
        ...
        vec3 res =  (1.0 - surface.material.metalness) * surface.material.baseColor;
        //imageStore(image, pixel, vec4(directDiffuse, 1.0));
        ...
        shadowFactor = PCSS_DirectionalLight(uniforms.light.shadow, cascadeMaps, surface.P,
            shadowNormal, saturate(dot(-uniforms.light.direction.xyz, shadowNormal)));
#endif
#ifdef SCREEN_SPACE_SHADOWS
        ...
        int clusteroffset = GetPointLightBufferOffset(surface.P);
        ...
        for(uint i = 0; i < pointLightUniforms.pointLightCount; i++)
        {
            ...
            Surface pointSurface = GetSurface(texCoord, depth, vec3(viewPointLightPos - surface.P), geometryNormal);

            vec3 pointDirectDiffuse = EvaluateDiffuseBRDF(pointSurface);
            vec3 pointDirectSpecular = EvaluateSpecularBRDF(pointSurface);

            float lightDistance = length(viewPointLightPos - surface.P);
            ...
            // PointShadow
            vec3 worldPos = vec3(globalData.ivMatrix  * vec4(surface.P, 1.0f) );
            ...
            direct += (pointDirectDiffuse + pointDirectSpecular) * Lradiance *  pointSurface.NdotL * pointShadowFactor; 
        }
